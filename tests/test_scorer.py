...
def make_config() -> UserConfig:
    return UserConfig(stack="python", seniority="senior", remote="remote")
...
async def test_score_parsed_correctly():
    ...
    score = await score_post(client, make_post(), "content", make_config(), "model")
...
async def test_content_truncated_to_4000_chars():
    ...
    await score_post(client, make_post(), long_content, make_config(), "model")
    call_kwargs = client.messages.create.call_args
    prompt = call_kwargs.kwargs["messages"][0]["content"]
    # The content in the prompt should be capped at 4000 chars
